...
A modern React application that processes large batches of `.txt`, `.md`, and `.docx` files using Google's Gemini AI with a queued, throttled workflow, and integrated Google Drive upload functionality.

## Features

- **Queue-Based Processing**: Upload any number of `.txt`, `.md`, or `.docx` files; processing runs in queued batches of 10 every 90 seconds
...
1. **Upload or Paste**: Select `.txt`/`.md`/`.docx` files or paste text to create a `.txt` input
